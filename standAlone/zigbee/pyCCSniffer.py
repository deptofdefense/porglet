...
class PacketHandler(object):
    ...
    def handleSniffedPacket(self, sniffedPacket):
        ...
        try:
            ...
            (rssiSniff, corr,
             crc_ok) = self.checkPacket(sniffedPacket.get_macPDU())
...
class CC2531EMK:
    ...
    def __init__(self, callback, channel=DEFAULT_CHANNEL):
        ...
        self.dev.ctrl_transfer(
            CC2531EMK.DIR_OUT, CC2531EMK.SET_POWER, wIndex=4)

        while True:
            # check if powered up
            power_status = self.dev.ctrl_transfer(CC2531EMK.DIR_IN,
                                                  CC2531EMK.GET_POWER, 0, 0, 1)
    ...
    def recv(self):
        ...
        while self.running:
            bytesteam = self.dev.read(
                CC2531EMK.DATA_EP, 4096, timeout=CC2531EMK.DATA_TIMEOUT)
...
#                         logger.warn("Received a command response with unknown code - CMD:%02x byte:%02x]" % (cmd, bytesteam[0]))
    ...
    def set_channel(self, channel):
        ...
        if 11 <= channel <= 26:
            ...
            # set channel command
            self.dev.ctrl_transfer(CC2531EMK.DIR_OUT, CC2531EMK.SET_CHAN, 0, 0,
                                   [channel])
            self.dev.ctrl_transfer(CC2531EMK.DIR_OUT, CC2531EMK.SET_CHAN, 0, 1,
                                   [0x00])
    ...
    def loopChannels(self):
        ''' temp method to loop through channels '''

        print("Channel loop started")

        while True:
            for ch in range(11, 27):
                self.set_channel(ch)

                print(f"\nSetting channel to: {ch}\n")
...
if __name__ == '__main__':
    ...
    threading.Thread(target=snifferDev.loopChannels, daemon=True).start()
