    ...
    for root, dirs, files in os.walk(path):
        for file in files:
            ...
            if file.endswith('.iq'):
                ...
                cs16File = open(basename + '.cs16', 'wb')
                tmpFile = open(basename + '.tmp', 'wb')
                cs8File = open(basename + '.cs8', 'wb')
                ...
                dataHackRF = data.astype(numpy.int8)

                print(f"Writing: {basename + '.cs16'}")
                cs16File.write(data.tobytes())

                print(f"Writing: {basename + '.tmp'}")
                tmpFile.write(dataFloat.tobytes())

                print(f"Writing: {basename + '.cs8'}")
                cs8File.write(dataHackRF.tobytes())
